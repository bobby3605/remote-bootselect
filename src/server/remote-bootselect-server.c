...
void construct_eth_hdr(struct ethhdr *frame, unsigned char *dest, unsigned char *src) {
    memcpy(frame->h_dest, dest, ETH_ALEN);
    memcpy(frame->h_source, src, ETH_ALEN);
    frame->h_proto = htons(BOOTSELECT_ETHERTYPE);
}

void send_packet(int sock, char *if_name, void *dst_addr, void *packet, size_t packet_len) {
    ...
    construct_eth_hdr((struct ethhdr *)frame, dst_addr, (unsigned char *)hwaddr.sa_data);
    memcpy(frame + sizeof(struct ethhdr), packet, packet_len);

    if (sendto(sock, frame, frame_size, 0, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
...
void request_default(int sock, char *if_name) {
    ...
    struct sockaddr hwaddr;
    get_if_info(sock, if_name, &ifindex, &hwaddr);
    while (memcmp(frame.hdr.h_dest, hwaddr.sa_data, sizeof(frame.hdr.h_dest)) != 0) {
...
void process_listen_socket(int listen_socket, char *if_name) {
    ...
    if (memcmp(frame.hdr.h_dest, ether_broadcast_addr, sizeof(ether_broadcast_addr)) != 0) {
        return;
    }
    ...
    struct RemoteDefaultData *data = find_default_data(*(struct uint48 *)frame.hdr.h_source);
    if (data) {
        send_packet(listen_socket, if_name, &data->mac, &data->default_entry, sizeof(data->default_entry));
...
void set_default_data(struct RemoteDefaultData *default_data) {
    for (unsigned int i = 0; i < remote_default_entries.size; ++i) {
        if (remote_default_entries.data[i].mac.x == default_data->mac.x) {
            memset(remote_default_entries.data[i].default_entry, 0, sizeof(remote_default_entries.data[i].default_entry));
            strcpy(remote_default_entries.data[i].default_entry, default_data->default_entry);
            return;
        }
    }
    ...
    struct RemoteDefaultData *new_data = malloc(sizeof(struct RemoteDefaultData) * ++remote_default_entries.size);
    memcpy(new_data, remote_default_entries.data, sizeof(struct RemoteDefaultData) * (remote_default_entries.size - 1));
    free(remote_default_entries.data);
    remote_default_entries.data = new_data;
    memcpy(&remote_default_entries.data[remote_default_entries.size - 1], default_data, sizeof(struct RemoteDefaultData));
}
